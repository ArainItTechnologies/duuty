...
const SelectRole = ({ onClose, setSelectedRole }) => {
  ...
  const handleSubmit = () => {
    setSelectedRole(activeRole); // Set the selected role in the parent component
  ...
  <div className="bg-white rounded-[20px] shadow-xl max-w-4xl w-full max-h-[90vh] p-8 z-50 relative flex flex-col">
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="grid grid-cols-2 justify-items-center sm:grid-cols-2 sm:justify-center sm:max-w-[400px] sm:m-auto lg:max-w-[100%] lg:grid-cols-4 lg:justify-start gap-4">
        {[
          ...
          <div
            key={role.id}
            className={`w-full sm:w-[150px] md:w-[190px] rounded-[10px] overflow-hidden border-[1.5px] ${
    ...
    <div className="sticky bottom-0 bg-white z-10 mt-4">
      <button
        className="bg-linear-(--gradient-bg) cursor-pointer block mx-auto bg-indigo-600 text-white px-6 py-2 rounded-[10px] hover:bg-indigo-700 transition"
